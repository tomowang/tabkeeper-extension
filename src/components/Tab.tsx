...
export default function Tab({tab, handleClickTabMenu, handleTabMouseEvent}: TabProps) {
  ...
  let img = <Image src={tab.favIconUrl} w={5} h={5} className={imageClassName}></Image>
  if (tab.tkFilter) {
    if (tab.tkMatched) {
      padding = 0
      boxShadowValue = `inset 0 0 0 99999px ${tab.tkColor}`
      img = <Image src={tab.favIconUrl} w={6} h={6} className={imageClassName}></Image>
    } else {
      img = <Image src={tab.favIconUrl} w={5} h={5} className={imageClassName} filter='auto' blur='1px'></Image>
    }
  }
  return <Popover trigger='click' placement='bottom-start'>
    <PopoverTrigger>
      <Flex borderWidth={1} borderRadius='base' alignItems='center' p={padding} cursor='pointer' borderColor={tab.active ? "blue.600" : ""} boxShadow={boxShadowValue}
